...
def processing(advanced, max_factor, model_path, image_path, model_name):
    img_size = 128
    ...
    if model_name == 'get_grading_from_image':
      LUT = img_to_lut(target[0], input_image[0])
    else:
...
def index():
    ...
    if (request.method == 'POST'):
        ...
        f.save(file_path)
        output = processing(advanced, max_factor, model_path, file_path, model_name)
